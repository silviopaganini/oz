...
(function() {
  var App, IFLLoader, IFLPhongFresnelShader, IFLStormParticleShader, NoiseShader,
  ...
  IFLStormParticleShader = (function() {
    ...
    IFLStormParticleShader.prototype.fragmentShader = ["uniform vec3 psColor;", "uniform float opacity;", "uniform float alpha;", "uniform sampler2D transparencyMap;", "varying vec3 vNWorld;", "varying vec3 vReflect;", "varying vec3 vNormal;", "varying vec3 vWorldPosition;", THREE.ShaderChunk["color_pars_fragment"], THREE.ShaderChunk["map_particle_pars_fragment"], THREE.ShaderChunk["fog_pars_fragment"], THREE.ShaderChunk["shadowmap_pars_fragment"], "#ifdef USE_ENVMAP", "uniform samplerCube envMap;", "#endif", "#ifdef USE_NORMALMAP", "uniform sampler2D normalMap;", "uniform vec2 normalScale;", "#endif", "void main() {", "gl_FragColor = vec4( psColor, opacity );", "#ifdef USE_ENVMAP", "vec3 reflectVec;", "#if defined( USE_BUMPMAP ) || defined( USE_NORMALMAP )", "vec3 mapN = texture2D( normalMap, vec2( gl_PointCoord.x, 1.0 - gl_PointCoord.y ) ).xyz;", "mapN.x = 1.0 - mapN.x;", "mapN = mapN * 2.0 - 1.0;", "vec3 normal = normalize( mapN );", "vec3 cameraToVertex = normalize( vWorldPosition - cameraPosition );", "reflectVec = reflect( cameraToVertex, normal );", "#else", "reflectVec = vReflect;", "#endif", "vec4 cubeColor = textureCube( envMap, vec3( -reflectVec.x, reflectVec.y, reflectVec.z ) );", "vec4 transColor = texture2D( transparencyMap, vec2( gl_PointCoord.x, 1.0 - gl_PointCoord.y ) );", "gl_FragColor = vec4( cubeColor.xyz * 2.0 ,transColor.r * 0.2);", "#endif", THREE.ShaderChunk["alphatest_fragment"], "}"].join("\n");
  ...
  IFLPhongFresnelShader = (function() {
    ...
    IFLPhongFresnelShader.prototype.vertexShader = ["#define PHONG", "uniform sampler2D tAux;", "uniform float mFresnelPower;", "uniform float mFresnelBias;", "uniform float mFresnelGain;", "uniform float mDisplacementScale;", "varying vec3 vViewPosition;", "varying vec3 vNormal;", "varying vec3 vNWorld;", "varying float vFresnel;", "varying vec3 vI;", THREE.ShaderChunk["map_pars_vertex"], THREE.ShaderChunk["lightmap_pars_vertex"], "varying vec3 vReflect;", "uniform float refractionRatio;", "uniform bool useRefract;", "varying vec3 vWorldPosition;", THREE.ShaderChunk["morphtarget_pars_vertex"], THREE.ShaderChunk["skinning_pars_vertex"], "void main() {", THREE.ShaderChunk["map_vertex"], THREE.ShaderChunk["lightmap_vertex"], THREE.ShaderChunk["morphnormal_vertex"], THREE.ShaderChunk["skinbase_vertex"], THREE.ShaderChunk["skinnormal_vertex"], THREE.ShaderChunk["defaultnormal_vertex"], "vNormal = transformedNormal;", THREE.ShaderChunk["morphtarget_vertex"], THREE.ShaderChunk["skinning_vertex"], "float dv = texture2D( tAux, uv ).x;", "float df = mDisplacementScale * dv;", "vec4 mvPosition = modelViewMatrix * vec4( position.xyz + normalize( normal ) * df, 1.0);", "gl_Position = projectionMatrix * mvPosition;", "vViewPosition = -mvPosition.xyz;", THREE.ShaderChunk["worldpos_vertex"], "#if defined( USE_ENVMAP )", "vNWorld = mat3( modelMatrix[ 0 ].xyz, modelMatrix[ 1 ].xyz, modelMatrix[ 2 ].xyz ) * objectNormal;", "vI = normalize( mPosition.xyz - cameraPosition );", "if ( useRefract ) {", "vReflect = refract( vI, normalize( vNWorld.xyz ), refractionRatio );", "} else {", "vReflect = reflect( vI, normalize( vNWorld.xyz ) );", "}", "#endif", "vWorldPosition = mPosition.xyz;", "}"].join("\n");

    IFLPhongFresnelShader.prototype.fragmentShader = ["uniform vec3 diffuse;", "uniform float opacity;", "uniform vec3 ambient;", "uniform vec3 emissive;", "uniform vec3 specular;", "uniform float shininess;", "uniform sampler2D tAux;", "uniform float mFresnelBias;", "uniform float mFresnelGain;", "uniform float mFresnelPower;", "varying vec3 vNWorld;", "varying vec3 vI;", THREE.ShaderChunk["map_pars_fragment"], THREE.ShaderChunk["lightmap_pars_fragment"], "#ifdef USE_ENVMAP", "varying vec3 vReflect;", "uniform float reflectivity;", "uniform samplerCube envMap;", "uniform float flipEnvMap;", "uniform int combine;", "#if defined( USE_BUMPMAP ) || defined( USE_NORMALMAP )", "uniform bool useRefract;", "uniform float refractionRatio;", "#endif", "#endif", THREE.ShaderChunk["fog_pars_fragment"], "varying vec3 vWorldPosition;", "varying vec3 vViewPosition;", "varying vec3 vNormal;", "varying float vFresnel;", THREE.ShaderChunk["shadowmap_pars_fragment"], THREE.ShaderChunk["specularmap_pars_fragment"], "void main() {", "gl_FragColor = vec4( diffuse, 0 );", "#ifdef USE_MAP", "#ifdef GAMMA_INPUT", "vec4 texelColor = texture2D( map, vUv );", "texelColor.xyz *= texelColor.xyz;", "gl_FragColor = gl_FragColor * texelColor;", "#else", "gl_FragColor = gl_FragColor * texture2D( map, vUv );", "gl_FragColor.xyz *= 1.5;", "#endif", "#endif", THREE.ShaderChunk["alphatest_fragment"], THREE.ShaderChunk["specularmap_fragment"], "vec3 viewPosition = normalize( vViewPosition );", "vec3 normal = normalize( vNormal );", "#ifdef DOUBLE_SIDED", "float flipNormal = ( -1.0 + 2.0 * float( gl_FrontFacing ) );", "#endif", "#ifdef USE_ENVMAP", "vec3 reflectVec;", "reflectVec = vReflect;", "#ifdef DOUBLE_SIDED", "vec4 cubeColor = textureCube( envMap, flipNormal * vec3( flipEnvMap * reflectVec.x, reflectVec.yz ) );", "#else", "vec4 cubeColor = textureCube( envMap, vec3( flipEnvMap * reflectVec.x, reflectVec.yz)  );", "#endif", "#ifdef GAMMA_INPUT", "cubeColor.xyz = cubeColor.xyz * cubeColor.xyz;", "#else", "cubeColor.xyz = cubeColor.xyz * 2.0;", "#endif", "gl_FragColor.xyz = mix( gl_FragColor.xyz, cubeColor.xyz , clamp(reflectivity,0.0,1.0)   );", "#endif", "float fresnel;", "#ifdef DOUBLE_SIDED", "fresnel = mFresnelBias + mFresnelGain * pow( 1.0 + dot( -viewPosition, normalize(flipNormal * normal) ), mFresnelPower );", "#else", "fresnel = mFresnelBias + mFresnelGain * pow( 1.0 + dot( -viewPosition, normal ), mFresnelPower );", "#endif", "gl_FragColor.w = clamp(1.0 - fresnel,0.0,1.0);", "#ifdef GAMMA_OUTPUT", "float d = 1.0/1.8;", "gl_FragColor.rgb = pow( gl_FragColor.rgb , vec3(d,d,d) );", "#endif", THREE.ShaderChunk["fog_fragment"], "}"].join("\n");
  ...
  IFLLoader = (function() {
    ...
    IFLLoader.prototype.convertGeometry = function(iflmesh, positions, uvs, normals, tangents, colors, color_length, isSkinnedMesh, skinWeights, skinIndices) {
      var cachedColors, cachedNormals, cachedPositions, cachedTangents, cachedUVs, face, faceIndex, fakeUV, fvUVs, geometry, i, i1, i12, i13, i14, i2, i22, i23, i24, i3, i32, i33, i34, ib, ibi, material, p1, p2, p3, positionRearrangment, second, stepSize, subMesh, subMeshIndex, _i, _j, _k, _l, _len, _len1, _ref, _ref1, _ref2, _step;
  ...
  App = (function() {
    ...
    App.prototype.cloudMeshes = null;

    App.prototype.clouds = null;

    App.prototype.instancedClouds = null;

    App.prototype.numClouds = 7;

    App.prototype.particles = [];

    App.prototype.cloudMinSpeed = 0.3;

    App.prototype.cloudMaxSpeed = 1.7;
    ...
    function App() {
      ...
      this.centroidSort = __bind(this.centroidSort, this);

      this.render = __bind(this.render, this);

      this.animate = __bind(this.animate, this);

      this.onCloudSpeedChange = __bind(this.onCloudSpeedChange, this);

      this.onCloudNumberChange = __bind(this.onCloudNumberChange, this);

      this.onDiffuseColorChange = __bind(this.onDiffuseColorChange, this);
    ...
    App.prototype.onTerrainLoaded = function(iflscene) {
      var balmat, centroid, chest, child, face, i, normal, particle, partmat, ran, s, _i, _j, _k, _len, _len1, _ref, _ref1,
        _this = this;
      this.clouds = [];
      this.cloudMeshes = [];
      this.instancedClouds = [];
      _ref = iflscene.children;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        child = _ref[_i];
        if (child && child.name.indexOf("cloud") !== -1) {
          child.geometry.computeCentroids();
          this.cloudMeshes.push(child);
          console.log("" + child.name + " added as template");
        }
        if (child && child.name.indexOf("storm") !== -1) {
          this.particleGeo = child.geometry;
        }
      }
      balmat = new THREE.MeshPhongMaterial({
        lights: true,
        color: 0xCCCCCC,
        ambient: 0x111111
      });
      this.baloon = new THREE.Object3D;
      this.baloon.add(new THREE.Mesh(new THREE.SphereGeometry(10, 10), balmat));
      ...
      this.gui.add(this, "numClouds", 1, 20).name("Number of clouds").step(1).onChange(this.onCloudNumberChange);
      this.gui.add(this.fresnel.uniforms["mFresnelPower"], "value", 0, 5).name("fresnel power");
      this.gui.add(this.fresnel.uniforms["mFresnelBias"], "value", 0, 1).name("fresnel bias");
      this.gui.add(this.fresnel.uniforms["mFresnelGain"], "value", 0, 10).name("fresnel gain");
      this.gui.add(this.fresnel.uniforms["reflectivity"], "value", 0, 1).name("reflectivity");
      this.gui.add(this.fresnel.uniforms["mDisplacementScale"], "value", 0, 100).name("Displ scale");
      this.gui.add(this, "noiseSpeed", 0, 1).name("Displ Speed");
      this.gui.addColor({
        val: [this.fresnel.uniforms.diffuse.value.r, this.fresnel.uniforms.diffuse.value.g, this.fresnel.uniforms.diffuse.value.b]
      }, "val", 0, 1).name("Diffuse Color").onChange(this.onDiffuseColorChange);
      this.gui.add(this.fresnel, "depthWrite").name("depthWrite");
      this.gui.add(this.fresnel, "depthTest").name("depthTest");
      this.gui.add(this.fresnel, "transparent").name("transparent");
      this.gui.add({
        side: this.fresnel.side === THREE.FrontSide
      }, "side").name("Backface Culling").onChange(function(value) {
        if (value) {
          return _this.fresnel.side = THREE.DoubleSide;
        } else {
          return _this.fresnel.side = THREE.FrontSide;
        }
      });
      this.gui.add(this.controls, "enabled").name("Enable Mouse Look");
      this.gui.add(this, "cloudMinSpeed", 0, 2).name("Cloud Min speed").onChange(this.onCloudSpeedChange);
      this.gui.add(this, "cloudMaxSpeed", 0, 5).name("Cloud Max speed").onChange(this.onCloudSpeedChange);
      $("#loading").remove();
      return null;
    };

    App.prototype.onDiffuseColorChange = function(value) {
      return this.fresnel.uniforms.diffuse.value.setRGB(value[0] / 255, value[1] / 255, value[2] / 255);
    };

    App.prototype.onCloudNumberChange = function(value) {
      var counter, k, m, mesh, oldcloud, s, _i, _j, _len, _len1, _ref, _ref1, _results;
      _ref = this.clouds;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        oldcloud = _ref[_i];
      ...
      this.clouds = [];
      _ref1 = this.cloudMeshes;
      _results = [];
      for (counter = _j = 0, _len1 = _ref1.length; _j < _len1; counter = ++_j) {
        mesh = _ref1[counter];
        if (!this.instancedClouds[counter]) {
          this.instancedClouds[counter] = [];
        }
        _results.push((function() {
          var _k, _results1;
          _results1 = [];
          for (k = _k = 0; 0 <= value ? _k < value : _k > value; k = 0 <= value ? ++_k : --_k) {
            if (!this.instancedClouds[counter][k]) {
              m = new THREE.Mesh(THREE.GeometryUtils.clone(mesh.geometry), mesh.material);
              m.material = this.fresnel;
              m.speed = this.cloudMinSpeed + Math.random() * (this.cloudMaxSpeed - this.cloudMinSpeed);
              m.rotation.set(0, Math.random() * Math.PI * 2, 0);
              m.position.set(0, -100 + Math.random() * 10, 0);
              s = 0.7 + Math.random() * .3;
              m.scale.set(s, 1, s);
              this.instancedClouds[counter][k] = m;
            }
            this.scene.add(this.instancedClouds[counter][k]);
            _results1.push(this.clouds.push(this.instancedClouds[counter][k]));
          }
          return _results1;
        }).call(this));
      }
    ...
    App.prototype.onCloudSpeedChange = function(value) {
      var cloud, _i, _len, _ref, _results;
      _ref = this.clouds;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        cloud = _ref[_i];
        _results.push(cloud.speed = this.cloudMinSpeed + Math.random() * (this.cloudMaxSpeed - this.cloudMinSpeed));
      }
      return _results;
    };

    App.prototype.createCloudMaterial = function() {
      ...
      uniforms["tAux"].value = this.noiseMap;
      this.fresnel = material;
      return material;
    };
    ...
    App.prototype.render = function() {
      var camX, camZ, cloud, delta, particSystem, r, t, _i, _j, _len, _len1, _ref, _ref1, _ref2, _ref3, _ref4;
      delta = this.clock.getDelta();
      this.renderer.clear();
      this.noiseShader.uniforms["time"].value += delta * this.noiseSpeed;
      this.noiseShader.uniforms["offset"].value.x += delta * this.noiseSpeed;
      ...
      for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
        particSystem = _ref1[_j];
        particSystem.rotation.y -= particSystem.speed * delta;
      }
      if (this.baloon) {
        t = this.clock.oldTime / 3000;
        this.baloon.position.x = Math.sin(t) * 150;
        if (!this.controls.enabled) {
          r = 50;
          camX = this.baloon.position.x + (r * Math.cos(t));
          camZ = this.baloon.position.z + (r * Math.sin(t) * 2);
          this.camera.position.set(camX, this.baloon.position.y, camZ);
          this.camera.lookAt(this.baloon.position);
        }
      }
      if ((_ref2 = this.sph) != null) {
        _ref2.position.x = Math.sin(this.clock.oldTime / 1600) * 200;
      }
      if ((_ref3 = this.controls) != null ? _ref3.enabled : void 0) {
        if ((_ref4 = this.controls) != null) {
          _ref4.update();
        }
      }
    ...
    App.prototype.centroidSort = function(a, b) {
      var a_dist, b_dist;
      a_dist = a.centroid.distanceTo(this.camera.position);
      b_dist = b.centroid.distanceTo(this.camera.position);
